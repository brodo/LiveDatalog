//! Repeated-query workload over a rule-heavy program. Before persistent
//! materialization every query rebuilds the complete derived closure, so this
//! benchmark makes the cost of query-time rebuilding visible in isolation.
...
pub fn main(init: std.process.Init) !void {
    ...
    var setup = try database.execute(
        ...
        \\node(X) :- source(X).
        \\node(X) :- sink(X).
        \\terminal(X) :- node(X), not source(X).
        \\origin(X) :- node(X), not sink(X).
        \\span(X, S) :- node(X), setof(Y, reachable(X, Y), S).
        \\length([], 0).
    ...
    try file_writer.interface.print(
        "{d} repeated queries over {d} rules on a {d}-node chain: {d} ns ({d} ns/query)\n",
        .{ iterations, 14, node_count, elapsed, elapsed / iterations },
    );
